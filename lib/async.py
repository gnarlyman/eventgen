...
class CallLater(object):
    ...
    def __init__(self, seconds, target, *args, **kwargs):
        ...
        if self._random is not None:
            if not isinstance(seconds, list):
                raise SyntaxError("seconds must be a list when random=True")
            self._random = self.seconds
            self._delay = random.choice(self._random)
        else:
            assert sys.maxint >= self.seconds >= 0, "%s is not greater than or equal " \
                                               "to 0 seconds" % seconds
            self._delay = self.seconds

        self._target = target(self, *args, **kwargs)
    ...
    def call(self):
        ...
        try:
            try:
                # pass self to the target, the target should handle this
    ...
    def cancel(self):
        ...
        assert not self.cancelled, "Already cancelled"
        self.cancelled = True
        del self._target, self._errback
        if self in _tasks:
